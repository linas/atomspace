...
class SQLAtomStorage::Response
{
	...
	public:
		...
		void exec(const char * buff, bool report_error=false)
		{
			if (rs) rs->release();
			...
			rs = _conn->exec(buff, report_error);
		}
		void exec(const std::string& str, bool report_error=false)
		{
			exec(str.c_str(), report_error);
		}
