...
/**
 * interrupt() - emulate (handle) user control-C fom keyboard.
 *
...
/**
 * hangup() - dettach the evaluation thread, and abandon output.
 *
 * Calling this will dettach the curently-running evaluator thread,
 * and ignore all output generated by that thread.  This also has the
 * side-effect of causing polling to finish.
 *
 * This is intended for use by the CogServer, for the case where it
 * starts some infinite loop via the shell interface, but then the user
 * disconnects from the shell (i.e. closes the tcpip socket to the shell).
 * The user's intent of such a disconnect is to leave the infinite loop
 * running, but discard any output it generates.
 *
 * The typical user scenario is to use netcat to start the loop, but
 * then abandon the netcat immediately.
 *
 * This method is kind-of ugly, and what it is really doing is to work
 * around a user-error: if the user wants to run a dettached thread,
 * they should do so explicitly. However, if the user fails to create
 * a dettached thread, and does close thier socket on an inf loop, then
 * ... then what? There are three choices:
 * A) Do nothing. Unfortunately, this leads to (unavoidable!?) deadlocks
 *    in the SchemeShell, as it waits for the command to complete.
 * B) Kill (interrupt) whatever the user started. But this is unfreindly
 *    to the casual netcat user on a busy system: we could be killing
 *    things that would terminate normally after a few seconds. Users
